...
public class LoginActivity extends UI implements OnKeyListener {
    ...
    private String id;
    private String account_tarot;
    private String password_tarot;
    ...
    private void setupRegisterPanel() {
        ...
        mTvForget = findView(R.id.register_forget_tip);
...
//                        }
    ...
    private void register() {
        ...
        RegistHttpClient.getInstance().register(account, code, password, new RegistHttpClient.RegistHttpCallback<RegistModel.DataBean>() {
            @Override
            public void onSuccess(RegistModel.DataBean data) {
                id = data.getId();
                switchMode();
...
//    }
    ...
    public void getSmsCode() {
        RegistHttpClient.getInstance().sendSmsCode(registerAccountEdit.getText().toString(), new RegistHttpClient.SendSmsHttpCallback<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                Toast.makeText(LoginActivity.this, "请留意短信", Toast.LENGTH_SHORT).show();
                mTimeDownBtn.startUp(60);
            }

            @Override
            public void onFailed(int code, String errorMsg) {
                Toast.makeText(LoginActivity.this, String.valueOf(code), Toast.LENGTH_LONG).show();
            }
        });
    }
...
//            mInputPhoneNumEditText.setError("请输入你的手机号");
            if (isNeedToast) {
                Toast.makeText(LoginActivity.this, "请输入你的手机号", Toast.LENGTH_SHORT).show();
            }
            return false;
        }
...
//                mInputPhoneNumEditText.setError("请输入正确的手机号");
            if (isNeedToast) {
                ...
                Toast.makeText(LoginActivity.this, "手机号不符合规范", Toast.LENGTH_SHORT).show();
            }
            return false;
        }
